...
if loadFromScratch:
    ...
    with open('saved_EnglishTrain23', 'w') as f:
        for item in english_sentences:
            f.write("%s\n" % item)
    with open('saved_FrenchTrain23', 'w') as f:
        for item in french_sentences:
            f.write("%s\n" % item)
    preproc_english_sentences, preproc_french_sentences, english_tokenizer, french_tokenizer = \
        preprocess(english_sentences, french_sentences, False)
    with open('pickle_english_tokenizer23.pickle', 'wb') as handle:
        pickle.dump(english_tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)
    with open('pickle_french_tokenizer23.pickle', 'wb') as handle:
        pickle.dump(french_tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)

else:
    with open('saved_EnglishTrain23', 'r') as f:
        english_sentences = f.read().split('\n')
    with open('saved_FrenchTrain23', 'r') as f:
        french_sentences = f.read().split('\n')
    with open('pickle_english_tokenizer23.pickle', 'rb') as handle:
        english_tokenizer = pickle.load(handle)
    with open('pickle_french_tokenizer23.pickle', 'rb') as handle:
